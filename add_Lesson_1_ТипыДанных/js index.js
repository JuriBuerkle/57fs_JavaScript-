...
/*условие: Выражение, которое вычисляется как true или false.
?: Разделитель, который отделяет условие от выражений, возвращаемых в зависимости от условия.
выражениеЕслиИстина: Выражение, которое будет выполнено и/или возвращено, если условие истинно (true).
:: Разделитель, который отделяет выражение для истинного случая от выражения для ложного.
выражениеЕслиЛожь: Выражение, которое будет выполнено и/или возвращено, если условие ложно (false).*/

//Примеры использования
//1. Присваивание значения переменной на основе условия


/*let возраст = 18;
let статус = (возраст >= 18) ? "Совершеннолетний" : "Несовершеннолетний";
console.log(статус); // Вывод: Совершеннолетний

возраст = 16;
статус = (возраст >= 18) ? "Совершеннолетний" : "Несовершеннолетний";
console.log(статус); // Вывод: Несовершеннолетний

//Это эквивалентно следующему if...else выражению:
...
console.log(статус);*/
//2. Использование внутри шаблонов строк (Template Literals)

let исправитьОшибка = true;
let сообщение = `Ваш запрос ${исправитьОшибка ? "был обработан" : "не удалось обработать"}.`;
console.log(сообщение); // Вывод: Ваш запрос был обработан.
//3. Возврат значения из функции

function получитьПриветствие(имя, естьЛиАвторизация) {
  return естьЛиАвторизация ? `Добро пожаловать, ${имя}!` : "Пожалуйста, авторизуйтесь.";
}

console.log(получитьПриветствие("Анна", true));  // Вывод: Добро пожаловать, Анна!
console.log(получитьПриветствие("Гость", false)); // Вывод: Пожалуйста, авторизуйтесь.
//4. Несколько тернарных операторов (вложенные) - использовать с осторожностью!
//Можно вкладывать тернарные операторы, но это может значительно снизить читабельность кода.

JavaScript

let оценка = 75;
let результат = (оценка >= 90) ? "Отлично" :
                (оценка >= 70) ? "Хорошо" :
                (оценка >= 50) ? "Удовлетворительно" :
                "Неудовлетворительно";
console.log(результат); // Вывод: Хорошо
